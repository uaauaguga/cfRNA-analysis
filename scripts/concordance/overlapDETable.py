...
parser.add_argument('--stat','-s',type=str,default=None,help="where to output the statistis")
parser.add_argument("--strip","-s",action="store_true",help="Whether strip gene ids",default=False)
...
df1up = df1[(df1["log2FoldChange"]>1)&(df1["padj"]<0.05)]
df2up = df2[(df2["log2FoldChange"]>1)&(df2["padj"]<0.05)]

df1down = df1[(df1["log2FoldChange"]<-1)&(df1["padj"]<0.05)]
df2down = df2[(df2["log2FoldChange"]<-1)&(df2["padj"]<0.05)]
...
columns = [args.label1+"-log2FoldChange",args.label1+"-padj",args.label2+"-log2FoldChange",args.label2+"-padj"]
res = pd.DataFrame(index=common,columns=columns)
res.loc[common,args.label1+"-log2FoldChange"] = df1.loc[common,"log2FoldChange"]
res.loc[common,args.label2+"-log2FoldChange"] = df2.loc[common,"log2FoldChange"]
...
stat.write("{} genes commonly down\n".format(len(commonDown)))
...
if args.options == "both":
    ...
    res = res.loc[:,[args.label1+"-padj",args.label1+"-log2FoldChange"]]
    res.columns = ["padj","log2FoldChange"]
    res.to_csv(args.output,sep="\t")
elif args.options == "second":
    res = res.loc[:,[args.label2+"-padj",args.label2+"-log2FoldChange"]]
    res.columns = ["padj","log2FoldChange"]
    res.to_csv(args.output,sep="\t")

stat.to_csv(args.stat,sep="\t".index_col=0)
