...
parser.add_argument("--bagging","-b",help="Whether use bagging for class unbalanced classification",default=False,action="store_true")
...
def SVM(X,y,bagging=False):
    if bagging:
        return BalancedBaggingClassifier(SVC(gamma='auto')).fit(X,y)
    else:
        return SVC(gamma='auto').fit(X,y)

def logit(X,y,bagging=False):
    if bagging:
        return BalancedBaggingClassifier(LogisticRegression(solver='lbfgs')).fit(X,y)
    else:
        return LogisticRegression(solver='lbfgs').fit(X,y)

def RF(X,y,bagging=False):
    if bagging:
        return BalancedBaggingClassifier(RandomForestClassifier()).fit(X,y)
    else:
        return RandonForestClassifier().fit(X,y)
...
def featureSelection(mat,pos,neg):
    ...
    for train_index, test_index in tqdm(sss.split(X,y)):
        ...
        if args.bagging:
            model = classiFun(X_selected[train_index,:],y[train_index],True)
        else:
