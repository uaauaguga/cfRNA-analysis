...
class BalancedBinaryClassifier(BaseEstimator, ClassifierMixin):
    def __init__(self,max_depth=None,n_features=10,selector=ranksum,trend="both",space_mask=None):
        ...
        self.model_ = BalancedRandomForestClassifier(max_depth=max_depth,n_estimators=100)
        self.trend = trend
    ...
    def fit(self, X, y):
        ...
        if self.trend == "both_balance":
            ...
            print(up_importance[:10],down_importance[:10])
            print(up_order[:n_up],down_order[:n_down])
